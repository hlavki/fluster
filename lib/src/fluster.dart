...
class Fluster<T extends Clusterable> {
  ...
  List<T> clusters(List<double> bbox, int zoom) {
    ...
    if (bbox[2] - bbox[0] >= 360) {
      ...
      List<T> westernHemisphere = clusters([-180, minLat, maxLng, maxLat], zoom);
    ...
    List<int> ids = tree!.range(_lngX(minLng), _latY(maxLat), _lngX(maxLng), _latY(minLat));
  ...
  PointCluster _createPointCluster(T feature, int id) {
    ...
    return PointCluster(x: x, y: y, zoom: 24, index: id, markerId: feature.markerId);
  }
  ...
  List<BaseCluster> _buildClusters(List<BaseCluster> points, int zoom) {
    ...
    for (int i = 0; i < points.length; i++) {
      ...
      String? childMarkerId = p.childMarkerId != null ? p.childMarkerId : p.markerId;
